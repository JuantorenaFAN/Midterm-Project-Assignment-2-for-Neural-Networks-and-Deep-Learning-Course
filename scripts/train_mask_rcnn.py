...
class ValidationHook(HookBase):
    ...
    def _do_eval(self):
        ...
        # Log segmentation metrics if available
        if results and "segm" in results:
            for k, v in results["segm"].items():
                self._tb_logger.log_scalar(f"val_segm/{k}", v, self.trainer.iter)
...
def setup_config(args):
    ...
    output_dir = os.path.join(args.output_dir, f"mask_rcnn_{timestamp}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train Mask R-CNN on VOC dataset")
...
def main():
    ...
    tb_logger = setup_tensorboard(cfg.OUTPUT_DIR, "mask_rcnn")
